...
import { arrayOf, normalize, Schema, unionOf, valuesOf } from '../src';

describe('normalize', () => {
  describe('throws when given', () => {
    describe('an input value of type', () => {
      [true, 42, null, undefined, '42'].forEach((input) => {
        it(`${typeof input}, ${JSON.stringify(input)}`, () => {
          assert.throws(() => normalize(input, {}));
        });
      });
    });

    describe('a schema value of type', () => {
      [[], true, 42, null, undefined, '42'].forEach((input) => {
        it(`${typeof input}, ${JSON.stringify(input)}`, () => {
          assert.throws(() => normalize({}, input));
        });
  ...
  describe('when given a single object input', () => {
    it('normalizes against a Schema', () => {
      const articleSchema = new Schema('articles');
      const input = deepFreeze({ id: 1, title: 'Some Article' });

      const result = normalize(input, articleSchema);
      assert.deepEqual(result, {
        result: 1,
        entities: {
          articles: {
            1: { id: 1, title: 'Some Article' }
          }
        }
      });
    });

    describe('normalizes when given a custom', () => {
      it('idAttribute value', () => {
        const articleSchema = new Schema('articles', { idAttribute: 'id_str' });
        const input = deepFreeze({ id_str: 1, title: 'Some Article' });
        ...
        assert.deepEqual(result, {
          ...
          entities: {
            articles: {
              1: { id_str: 1, title: 'Some Article' }
            }
          }
        });
      });

      it('idAttribute function', () => {
        const articleSchema = new Schema('articles', { idAttribute: (input) => {
          return input[input.idField];
        } });
        const input = deepFreeze({ fooId: 1, title: 'Some Article', idField: 'fooId' });
        ...
        assert.deepEqual(result, {
          ...
          entities: {
            articles: {
              1: { fooId: 1, title: 'Some Article', idField: 'fooId' }
            }
          }
        });
      });
    ...
    describe('when given a custom assignEntity method', () => {
      it('can modify the output object', () => {

      });

      it('has access to the original input object', () => {

      });

      it('has access to the schema for the current entity', () => {

      });

      it('runs the custom method after the global method', () => {

      });
    });
  });

  describe('when given an array input', () => {
    it('can normalize against a single array of Schema', () => {
      const articleSchema = new Schema('articles');
      ...
      const result = normalize(input, arrayOf(articleSchema));
      assert.deepEqual(result, {
        result: [3, 1, 2],
        entities: {
          articles: {
            1: { id: 1, title: 'Tacos' },
            2: { id: 2, title: 'Burritos' },
            3: { id: 3, title: 'Churros' }
          }
        }
      });
    });

    describe('can normalize multiple Schema', () => {
      const input = deepFreeze([
        { id: 4, name: 'Tacos', type: 'main' },
        { id: 1, name: 'Fries', type: 'side' },
        { id: 4, name: 'Salad', type: 'side' },
        { id: 2, name: 'Salad', type: 'main' }
      ]);

      it('with a string value', () => {
        const mainSchema = new Schema('mains');
        const sideSchema = new Schema('sides');
        const menuItems = {
          // Note: When using a string value for `schemaAttribute`,
          // this must be mapped as [schemAttribute value]: schema
          main: mainSchema,
          side: sideSchema
        };

        const output = normalize(input, arrayOf(menuItems, { schemaAttribute: 'type' }));
        ...
        assert.deepEqual(output, {
          ...
          entities: {
            mains: {
              4: { id: 4, name: 'Tacos', type: 'main' },
              2: { id: 2, name: 'Salad', type: 'main' }
            },
            sides: {
              1: { id: 1, name: 'Fries', type: 'side' },
              4: { id: 4, name: 'Salad', type: 'side' }
            }
          }
        });
      });

      it('and infer them with a callback function', () => {
        const entreeSchema = new Schema('entrees');
        const appetizerSchema = new Schema('appetizers');
        const menuItems = {
          entree: entreeSchema,
          appetizer: appetizerSchema
        };

        const output = normalize(input, arrayOf(menuItems, { schemaAttribute: (entity) => {
          switch (entity.type) {
            case 'main':
              return 'entree';
            case 'side':
        ...
        assert.deepEqual(output, {
          result: [
            { id: 4, schema: 'entree' },
            { id: 1, schema: 'appetizer' },
            { id: 4, schema: 'appetizer' },
            { id: 2, schema: 'entree' }
          ],
          entities: {
            entrees: {
