...
exports.keyedObjectOf = keyedObjectOf;
...
var _KeyedObjectSchema = require('./KeyedObjectSchema.js');

var _KeyedObjectSchema2 = _interopRequireDefault(_KeyedObjectSchema);
...
function visitKeyedObject(obj, schema, bag, options) {
  var keyedObjectStoredKeyName = schema.getKeyedObjectStoredKeyName();
  var itemSchema = schema.getItemSchema();

  return Object.keys(obj).reduce(function (objMap, key) {
    var entry = obj[key];
    entry[keyedObjectStoredKeyName] = key;
    var storageId = itemSchema.getId(entry, key);
    var entity = visitEntity(entry, itemSchema, bag, options, storageId);
    objMap[key] = entity;
    return objMap;
  }, {});
}
...
function visit(obj, schema, bag, options, collectionKey) {
  ...
  } else if (schema instanceof _KeyedObjectSchema2.default) {
...
function keyedObjectOf(schema, options) {
  return new _KeyedObjectSchema2.default(schema, options);
}
